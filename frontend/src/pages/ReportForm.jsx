...
export default function ReportForm() {
  ...
  const canSubmit = loc.trim() && desc.trim() && phoneValid && photo

  const handlePhoneChange = (val) => {
    setPhone(val)

    if (val.length > 0) {
      setPhoneError(!/^\d{10}$/.test(val))
    } else {
      setPhoneError(false)
    }
  }
  ...
  const detectGPS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        if (err.code === 1) {
          setLocStatus(
            '⚠️ Location permission denied. Please type manually.'
          )
        } else {
          fetch('https://ipapi.co/json/')
            ...
            .then((d) => {
              if (d.latitude) {
                ...
                setLocStatus(
                  '⚠️ Could not detect location. Please type manually.'
                )
              }
            })
            .catch(() =>
  ...
  const handlePhoto = (e) => {
    ...
    // Keep a preview for displaying in the UI
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('issueType', selectedCategory.title)
        ...
        >
          ...
          <div
            style={{
              ...
              animation:
                'fadeIn 0.5s ease-out 0.1s both',
            }}
          >
            {/* Description */}
            ...
            <button
              ...
              onMouseEnter={() =>
                canSubmit && !isSubmitting && hov(true)
              }
              onMouseLeave={() => hov(false)}
            >
              {isSubmitting
